...
def createnewwallet(request):
    ...
    return HttpResponse(json.dumps(data), content_type = "application/json") #datayı json ile stringe cevirirz
...
@csrf_exempt #csrf korumasını kaldırmak
def checkwallet(request):
    data = {}
    prikey = request.POST.get('prikey').strip() #strip bosluk varsa kaldırır
    pubkey = request.POST.get('pubkey').strip()
    message = b"hello"
    try:
        #In video many old codes here
        #this is login process with rsa key of customer keys
        mypubkey = base64.b64decode(pubkey)
        myprikey = base64.b64decode(prikey)
        mykey = RSA.importKey(myprikey.decode('utf-8'))
        example=text = "hello world".encode('utf-8')
        cipher = PKCS1_V1_5.new(mykey)
        myencdata = mykey.encrypt(example_text, cipher)
        mydecodedata=mykey.decrpy(myencdata)
